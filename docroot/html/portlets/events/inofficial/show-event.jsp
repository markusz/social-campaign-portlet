...
<aui:panel>
	...
	<span><liferay-ui:message key="events-user-attends-event" />(<a
		href="<%=unsubscribeFromEvent.toString()%>"><liferay-ui:message
				key="events-unsubscribe-from-event" />
	</a>)</span>
	<br>
	<%
	...
	%>
	<a href="<%=makeEventOfficial.toString()%>"><liferay-ui:message
			key="events-make-event-official" />
	</a>
	<%
		}
	%>
...
<table border="0">
	<tr>
		<td style="padding: 10px 70px 50px 20px; vertical-align:top">
			...
			<a
				...
				<tr>
					...
					<td id="p2"><img style="height: 60px;"
						...
						<div><%=eventCreator.getFullName()%>
							<%
								if (PermissionsUtil.isCampaignLeader(eventCreator)) {
							%><br>
				...
				<tr>
					<td id="p1"><b><liferay-ui:message
								key="events-attending-users" /> (<%=attendences.size()%>) </b></td>
 ...
 %>
		...
		<td style="padding: 20px; vertical-align: top"><iframe
				width="425" height="200" frameborder="0" scrolling="no"
